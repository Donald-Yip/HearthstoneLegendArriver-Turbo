...
> 🖼️ 网页一键画出**所有截图区域框**，确认盒子 UI 有没有放进绿框
...
| 🎯 校准区域 / 截图区域框 | 屏幕上拖绿框校准；网页内一键把**所有截图区域**画在截图上，确认分辨率与盒子 UI 位置 |
...
### 🖼️ 网页内「显示截图区域框」（不用拖框，先看一眼）

不想先开校准工具的话，点网页「🎯 校准区域 / 截图区域框」里的 **[🖼️ 显示截图区域框（页面内）]**：脚本会截一张当前屏幕，把**所有实际用到的截图区域**画出来。

| 画出来的框 / 点 | 对应区域 |
| --- | --- |
| 🟩 绿框 | 盒子推荐面板（OCR 识别来源 `recommendation_roi`） |
| 🟦 蓝框 | 换牌「确认」按钮区域（提交校验用） |
| 🟧 橙框 | 盒子「AI胜率」浮动条（自动投降用） |
| 🟥 红细框 | AI胜率兜底区域（主区域读不到时放宽再读一次） |
| ✛ 黄十字 | 阶段判定点（脚本靠这几个像素判断当前是主界面/选牌/对战中） |

图片下面同时给结论：**分辨率对不对、缩放是不是 100%、绿框里有没有盒子面板**。对局中若提示「绿框里没有检测到盒子面板」，就把盒子面板拖进绿框再看一次（这一步等于确认「盒子 UI 有没有摆正」）。

整个过程**只截图，不点击、不移动鼠标**，自动化运行中也能安全使用。

<img width="1180" alt="截图区域框" src="docs/images/regions.png" />
...
**第一次启动脚本会自动自检一遍**，之后随时可在网页顶部「🧪 环境自检」卡片点 [🔍 重新自检]。
